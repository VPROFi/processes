...
void progress::show()
{
	if (!_visible) {
		_visible = true;
		Plugin::psi.AdvControl(Plugin::psi.ModuleNumber, ACTL_SETPROGRESSSTATE, (void*)PGS_INDETERMINATE, NULL);
	}
...
{
	if (_visible) {
		Plugin::psi.AdvControl(Plugin::psi.ModuleNumber, ACTL_PROGRESSNOTIFY, NULL, NULL);
		Plugin::psi.AdvControl(Plugin::psi.ModuleNumber, ACTL_SETPROGRESSSTATE, (void*)PGS_NOPROGRESS, NULL);
...
void progress::update(const uint64_t val)
{
	...
	pv.Total = 100;
	Plugin::psi.AdvControl(Plugin::psi.ModuleNumber, ACTL_SETPROGRESSVALUE, &pv, NULL);
